...
import play.api.mvc.{RequestHeader, Call, Action}
import play.api.libs.concurrent.Execution.Implicits.defaultContext

import scala.concurrent.Future

@Singleton class OAuth @Inject() (service: Service) extends Controller(service) {
  ...
  val callback = Action.async { implicit request =>
    request.getQueryString("code") match {
      case Some(code) =>
        ...
        for {
          response <- service.ws.url(tokenEndpoint).post(Map(
            "code" -> Seq(code),
